...
- Added `switchAli` (default `True`): the tracking target is re-anchored at the first branch switch, when all tilt angles are still small. The script tilts back to the `startTilt`, aligns a View image to the View reference saved during target selection, then tilts to the switch tilt and aligns again to both the View and the target (Preview) reference, keeps the better matching alignment and checks the residuals against `tgtAlignTol`. This corrects the image shift drift accumulated on the first branch before the tilt direction reverses and directly addresses the large off-target usually observed at the first branch switch.
</details>
