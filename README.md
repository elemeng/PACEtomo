...
| `fineZ` | `True` | Perform the fine residual Z correction at the tracking target before the acquisition. Set to `False` to skip *all* eucentric Z adjustment by this script — the coarse `sem.Eucentricity()` routine is permanently removed and will not be used as a fallback, so with `fineZ = False` the lamella eucentric height must be established before the map (e.g. with the *z_by_v* script). |
| `fineZTol` | `0.5` | Convergence tolerance [µm] of the residual Z correction that is performed at the tracking target before the acquisition. This is a target-local correction only: the lamella eucentric height should be established *before* taking the map (e.g. with the *z_by_v* script), and this correction refines the tracking target to that height without re-establishing it. The refined Z is written back into the tracking Navigator item only, and the total applied correction is reported in the log. |
...
| `switchAli` | `True` | Re-anchor the tracking target at the first branch switch of the tilt series, when all tilt angles are still small: the script tilts back to the `startTilt`, aligns a View image to the View reference saved during target selection, then tilts to the switch tilt and aligns again to both the View and the target (Preview) reference. The residuals are checked against `tgtAlignTol` and the better matching alignment is kept, so that the image shift drift accumulated on the first branch is corrected before the tilt direction reverses. The re-anchor correction is propagated to the other targets (shared drift), so they follow the corrected tracking reference. Adds one View and one Preview image on the tracking area; on stages with a large first-switch off-target this re-anchoring replaces the error-prone large swing across the `startTilt`. |
...
| `imageShiftLimit` | `15` | SerialEM has a hard limit on applying image shifts, which is 15 µm by default. `imageShiftLimit` will overwrite this SerialEM property. The maximum amount of image shift is system dependent and the limit for Thermo Scientific TEM systems is always somewhere below 25 µm. During target setup, targets whose measured image shift (relative to the tracking position) exceeds this limit minus `alignLimit` are skipped, so no target starts a series that will abort mid-branch. |
...
| `resetIS_AlignTo_Limit` | `0.5` | Maximum residual image shift [µm] allowed at the tracking target after the initial realignment. If exceeded, the image shift is reset (stage move via `ResetImageShift`) and the target realigned with View until the IS is below this limit (up to 10 attempts). If the limit cannot be reached, the IS needed for the alignment is kept and the acquisition continues anyway, so that at least targets close to the tracking position can be collected at the defined positions. This runs once per exposure group (fresh runs and recovery) so that the image shifts of the other targets in the group stay within `imageShiftLimit`. |
| `tgtAlignTol` | `200` | Maximum residual alignment error [nm] allowed when centering a target at the `startTilt` during target setup. If the error exceeds the tolerance, the alignment is retried once (the image shift is reset and the target shift re-applied); if it still fails, the target is marked for skipping (for the tracking target only a warning is issued). Catches targets whose saved reference can no longer be aligned reliably. |
| `maxAlignError` | `200` | Maximum accumulated alignment error [nm] allowed for a target during the tilt series, relative to the alignment at the `startTilt` reference. For a data target, its branch is aborted when the error exceeds this limit, so that no off-target images are collected at high tilts; for the tracking target a warning is issued (including when the error persists over consecutive tilts), since large tracking corrections indicate that the image shift limit may be reached for all targets. |
...
Before acquiring each area, the script performs a **fine eucentric Z refinement** at the tracking target (`fineZ = True`, default): it measures the defocus with SerialEM's autofocus in the Record low-dose area (beam-tilt based, like the fine step of the *z_by_v* script) and moves the stage Z by the defocus error until it is within `fineZTol`, then writes the refined Z into the tracking Navigator item. This replaces the coarse `sem.Eucentricity()` routine and keeps the targets centred on the positions you defined. With `fineZ = False` this correction is skipped entirely (the coarse routine is never used as a fallback). The **recommended workflow** is to correct the eucentric height of the lamella *before* taking the map on which you add the targets (e.g. using SerialEM's eucentricity function or the [*PACEtomo_Z_by_V.py*](#pacetomo_z_by_vpy) beam-tilt compensation script before a lamella montage) — PACEtomo then only needs to do a quick fine correction before each tilt series.
...
This is an eucentric Z correction script. It adjusts the stage Z to the eucentric height using the beam-tilt autofocus defocus measurement: first a coarse correction in the View low-dose area using its stored parameters, then a fine correction either in the same View area (stored parameters, `fineMag = 0`, default) or in the Record area (`fineMag` non-zero, using the stored Record configuration with the high mag). No magnification is set in the script — the autofocus re-applies the low-dose area's stored parameters, so the mag, beam, defocus and intensity come from the area configuration. It is meant to be run on the lamella *before* taking a map (e.g. a lamella montage) so that the targets you add on the map are at an already-corrected eucentric Z — PACEtomo will then only perform a quick fine correction before each tilt series.
- How to use:
...
### 17.09.2026
#### PACEtomo.py [v1.9.3]
Monitoring of alignment errors during target setup and tilt series acquisition.
<details>
<summary>Changes</summary>

- Added `tgtAlignTol` (default 200 nm): the residual alignment error of every target is checked after the initial centering at the `startTilt`. If it exceeds the tolerance, the alignment is retried once (image shift reset and target shift re-applied) and, if it still fails, the target is marked for skipping. For the tracking target only a warning is issued, since the acquisition depends on it.
- Added `maxAlignError` (default 200 nm): the accumulated alignment error of every target is checked at every tilt (this value was previously only written to the log). If a data target exceeds the limit, its branch is aborted so that no off-target images are collected at high tilts. For the tracking target a warning is issued, including when the error persists over consecutive tilts, because large tracking corrections also indicate that the image shift limit of the other targets may be reached.
- Added `switchAli` (default `True`): the tracking target is re-anchored at the first branch switch, when all tilt angles are still small. The script tilts back to the `startTilt`, aligns a View image to the View reference saved during target selection, then tilts to the switch tilt and aligns again to both the View and the target (Preview) reference. The Preview reference is the fixed anchor, the View reference is an independent quality check, and an agreement/disagreement between the two is reported (an explicit fallback to the View alignment is only made when the Preview fails and the View is within `tgtAlignTol`). The residuals are checked against `tgtAlignTol` and the re-anchored image shift is stored, correcting the drift accumulated on the first branch before the tilt direction reverses.
- Per-tilt logging now separates the prediction from the measurement, the applied correction and the post-alignment residual (`Prediction`, `Reality`, `Correction`, `Post-alignment residual` lines), so that a large correction with a small residual (stage displaced, tracking corrected it) can be distinguished from a bad tracking result. Tracking-target warnings show the consecutive-off-target streak (`streak N/3`).
- The `switchAli` re-anchor correction is now propagated to the other targets (both branches), so they follow the corrected tracking reference instead of only the tracking target. In addition, targets whose measured image shift during target setup exceeds `imageShiftLimit` minus `alignLimit` are skipped before the series starts (the tracking target only warns).
</details>
...
- Replaced the coarse `sem.Eucentricity()` routine at the tracking target with a fine eucentric Z refinement: the script measures the defocus with SerialEM's autofocus in the Record low-dose area (beam-tilt based, like the fine step of the *z_by_v* script) and moves the stage Z by the defocus error until it is within `fineZTol` (default 0.5 µm), then writes the refined Z into the tracking Navigator item. The coarse routine could leave the target up to ~3 µm off, contributing to off-target acquisition.
- Added `fineZTol` and `eucentricTargetDefocus` settings.
- Added `fineZ` setting (default `True`): the fine residual Z correction at the tracking target can now be switched off. With `fineZ = False` the script performs *no* eucentric Z adjustment, and the coarse `sem.Eucentricity()` routine is never used as a fallback — the lamella eucentric height must then be established before the map (e.g. with the *z_by_v* script).
...
- New standalone Python version of the *z_by_v* eucentric Z correction script (no dependency on external SerialEM function libraries). Two steps: coarse in the View low-dose area (using its stored parameters), fine in the same area or in the Record area (`fineMag`); no magnification is set in the script; beam tilt amplitude controlled via the `beamTilt` setting (`SetUserSetting AutofocusBeamTilt`).
</details>
