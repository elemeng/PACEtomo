...
| `reAlignISLimit` | `0.5` | Maximum residual image shift [µm] allowed at the tracking target after the initial realignment. If exceeded, the image shift is reset (stage move via `ResetImageShift`) and the target realigned with View until the IS is below this limit, so that the image shifts of the other targets in the group stay within `imageShiftLimit`. |
